...
class GQAImagesDataset(Dataset):

    def __init__(self, gqa_dir: str, visual_input_type: str, image_size=(384,640)):

        '''
        Initializes an GQAImage instance that handles image-side processing for GQA
        gqa_dir: directory that contains MS-COCO data (images within 'images' folder)
        visual_input_type: format of visual input to model
        image_size: tuple indicating size of image input to model
        '''

        self.images_dir = gqa_dir        # Images across all 2017 splits stored in same directory
...
if __name__ == '__main__':
    ...
    dataset = GQAImagesDataset('/home/shared/MCL/gqa/images', 'raw')
