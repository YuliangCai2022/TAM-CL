...
import pickle
import pdb
from PIL import Image, ImageFile  # add by Yuliang
#ImageFile.LOAD_TRUNCATED_IMAGES = True # add by Yuliang
...
class NLVR2Dataset(Dataset):
    ...
    def __init__(self, 
        ...
        if False: #os.path.exists(self.cached_data_file): # modified by Yuliang
            ...
            print("annotations_file is " + str(annotations_file))
            self.data = []
            i = 0
            ...
            with jsonlines.open(annotations_file) as reader:
                for annotation in reader:
                    ...
                    '''if not os.path.exists(example["image_id_0"]): # added by Yuliang
                        continue 
                    else:
                        try:
                            img=Image.open(example["image_id_0"])
                        except:
                            print('except 0')
                            continue
                        else:
                    ...
                    '''if not os.path.exists(example["image_id_1"]): # added by Yuliang
                        continue 
                    else:
                        try:
                            print('except 1')
                            img=Image.open(example["image_id_1"])
                        except:
                            continue
                        else:
                            pass'''
                    ...
                    self.data.append(example)  ## added by Yuliang
            with open(self.cached_data_file, 'wb') as f:
                pickle.dump(self.data, f)
        print("n_examples is " + str(len(self.data)))
