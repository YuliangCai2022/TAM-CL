...
class OKVQADataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.annotations_file = os.path.join(data_dir, 'mscoco_{}2014_annotations.json'.format(split))
        self.questions_file = os.path.join(data_dir, 'OpenEnded_mscoco_{}2014_questions.json'.format(split))
        ...
        cache_dir = os.path.join(data_dir, 'cached_okvqa_data')
        if not os.path.exists(cache_dir):
            os.makedirs(cache_dir)
        self.cached_data_file = os.path.join(cache_dir, 'okvqa_{}.pkl'.format(split))
        if os.path.exists(self.cached_data_file):
            ...
            annotations = json.load(open(self.annotations_file,encoding='utf-8'))['annotations']
            self.data = []
        ...
        logger.info("Loaded OKVQA {} dataset, with {} examples".format(self.split, len(self.data)))
...
def okvqa_batch_collate(batch: List[Dict], 
...
def build_okvqa_dataloader(args, 
                         data_dir: str, 
                         images_dataset: MSCOCOImagesDataset, 
                         split: str, 
                         visual_input_type: str,
    ...
    logger.info("Creating OKVQA {} dataloader with batch size of {}".format(split, batch_size))

    dataset = OKVQADataset(data_dir, images_dataset, split, **kwargs)
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        shuffle=shuffle,
        collate_fn=lambda x: okvqa_batch_collate(x, visual_input_type))
    return dataloader

if __name__ == '__main__':
    data_dir = '/data/datasets/MCL/okvqa/'
    ...
    okvqa_dataloader = build_okvqa_dataloader(args, data_dir, images_dataset, 'val', args.visual_input_type, tokenizer=tokenizer)

    for batch in okvqa_dataloader:
        pdb.set_trace() 