...
from data.visionlanguage_datasets.vqaabs_dataset import build_vqaabs_dataloader
...
class VQAAbstractTrainer(TaskTrainer):
    ...
    def __init__(self, 
        ...
        self.vqaabs_config = task_configs['vqaabs']
        self.data_dir = os.path.join(args.climb_data_dir, self.vqaabs_config['data_dir'])
        ...
        self.vqaabs_train_dataloader = build_vqaabs_dataloader(args=args,
                                                    data_dir=self.data_dir,
                                                    split='train',
                                                    visual_input_type=self.visual_input_type)

        self.vqaabs_val_dataloader = build_vqaabs_dataloader(args=args,
                                                  data_dir=self.data_dir,
                                                  split='val',
                                                  visual_input_type=self.visual_input_type)

        # Training hyperparameters
        self.num_epochs = self.vqaabs_config['num_epochs']
        self.lr = self.vqaabs_config['lr']
        self.adam_epsilon = self.vqaabs_config['adam_epsilon']
        self.weight_decay = self.vqaabs_config['weight_decay']
        self.hparams = {
                        'lr': self.lr,
        ...
        self.max_steps = len(self.vqaabs_train_dataloader) * self.num_epochs
    ...
    def get_train_dataloader(self):
        return self.vqaabs_train_dataloader

    def get_collate_fn(self):
        return self.vqaabs_train_dataloader.collate_fn
    ...
    def forward_pass(self, model, batch: Dict, do_eval: bool = False) -> Tuple:
        ...
        if do_eval is True:
            with torch.no_grad():
                output = model(task_key='vqaabs', **inputs)
        else:
            output = model(task_key='vqaabs', **inputs)
        ...
        if self.args.dytox != 0 and replay != 1:
            if self.args.parallel != 0:
                ...
                if model.teacher_model != None and self.args.task_attention:
                    ...
                    output_old_origin = model.teacher_model(task_key=self.args.ordered_cl_tasks[self.num_task-2],teacher_key = 'vqaabs', **old_inputs)
    ...
    def train(self, model, replay_memory=None, ewc=None) -> Tuple[float, Dict]:
        ...
        for epoch in range(self.num_epochs):
            ...
            model.train()
            for step, batch in enumerate(tqdm(self.vqaabs_train_dataloader, desc='Training epoch {}'.format(epoch+1))):
                ...
                if (step + 1) % wandb_logger.get_log_freq() == 0:
                    log_dict = {'vqaabs': {'loss': loss.item()}}
            ...
            wandb_logger.log({'vqaabs': {'val_score': eval_score}})
    ...
    def eval(self, model) -> float:
        ...
        for step, batch in enumerate(tqdm(self.vqaabs_val_dataloader, desc='Evaluating on VQA-Abstract val set')):
        ...
        eval_score = eval_score/len(self.vqaabs_val_dataloader.dataset)*100.0
...
class LowShotVQAAbstractTrainer(VQAAbstractTrainer):
    ...
    def __init__(self,
        ...
        super(LowShotVQAAbstractTrainer, self).__init__(args, task_configs, model_config)
        self.low_shot_config = low_shot_config
        self.eval_epochs = [x-1 for x in low_shot_config['eval_epochs']]

        self.vqaabs_train_dataloader.dataset.convert_to_low_shot(low_shot_percentage=low_shot_config['percentage'])
        self.max_steps = len(self.vqaabs_train_dataloader) * self.num_epochs
