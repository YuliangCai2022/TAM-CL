...
from data.visionlanguage_datasets.pathvqa_dataset import build_pathvqa_dataloader
...
class PathVQATrainer(TaskTrainer):
    ...
    def __init__(self, 
        ...
        '''
        Initializes a Trainer that handles training of a model on the PathVQA task
        ...
        self.pathvqa_config = task_configs['pathvqa']
        self.data_dir = os.path.join(args.climb_data_dir, self.pathvqa_config['data_dir'])
        ...
        self.pathvqa_train_dataloader = build_pathvqa_dataloader(args=args,
                                                    data_dir=self.data_dir,
                                                    split='train',
                                                    visual_input_type=self.visual_input_type)

        self.pathvqa_val_dataloader = build_pathvqa_dataloader(args=args,
                                                  data_dir=self.data_dir,
                                                  split='val',
                                                  visual_input_type=self.visual_input_type)

        # Training hyperparameters
        self.num_epochs = self.pathvqa_config['num_epochs']
        self.lr = self.pathvqa_config['lr']
        self.adam_epsilon = self.pathvqa_config['adam_epsilon']
        self.weight_decay = self.pathvqa_config['weight_decay']
        self.hparams = {
                        'lr': self.lr,
        ...
        self.max_steps = len(self.pathvqa_train_dataloader) * self.num_epochs
        self.warmup_ratio = 0.1 # TODO remove hard code
    
    def get_train_dataloader(self):
        return self.pathvqa_train_dataloader

    def get_collate_fn(self):
        return self.pathvqa_train_dataloader.collate_fn
    ...
    def forward_pass(self, model, batch: Dict, do_eval: bool = False) -> Tuple:
        ...
        if do_eval is True:
            with torch.no_grad():
                output = model(task_key='pathvqa', **inputs)
        else:
            output = model(task_key='pathvqa', **inputs)
    ...
    def train(self, model, replay_memory=None, ewc=None) -> Tuple[float, Dict]:
        ...
        for epoch in range(self.num_epochs):
            ...
            model.train()
            for step, batch in enumerate(tqdm(self.pathvqa_train_dataloader, desc='Training epoch {}'.format(epoch+1))):
                ...
                if (step + 1) % wandb_logger.get_log_freq() == 0:
                    log_dict = {'pathvqa': {'loss': loss.item()}}
            ...
            wandb_logger.log({'pathvqa': {'val_score': eval_score}})
    ...
    def eval(self, model) -> float:
        ...
        for step, batch in enumerate(tqdm(self.pathvqa_val_dataloader, desc='Evaluating on pathvqa val set')):
        ...
        eval_score = eval_score/len(self.pathvqa_val_dataloader.dataset)*100.0
...
class LowShotPathVQATrainer(PathVQATrainer):
    ...
    def __init__(self,
        ...
        '''
        Creates instance of low-shot PathVQA trainer according to low_shot_config
        ...
        super(LowShotPathVQATrainer, self).__init__(args, task_configs, model_config)
        self.low_shot_config = low_shot_config
        self.eval_epochs = [x-1 for x in low_shot_config['eval_epochs']]

        self.pathvqa_train_dataloader.dataset.convert_to_low_shot(low_shot_percentage=low_shot_config['percentage'])
        self.max_steps = len(self.pathvqa_train_dataloader) * self.num_epochs

    def train(self, model):
        '''
        Trains model on low-shot PathVQA task
        Args:
        model
